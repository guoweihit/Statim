...
def get_proc_env(os_env=None):
    ...
    if pymoddir is not None:
        python_paths.append(pymoddir.get_bld().abspath())
    pyvizdir = bld.path.find_dir('src/visualizer')
    if pyvizdir is not None:
        python_paths.append(pyvizdir.abspath())
    if python_paths:
        if 'PYTHONPATH' in proc_env:
            python_paths.append(proc_env['PYTHONPATH'])
        proc_env['PYTHONPATH'] = os.pathsep.join(python_paths)
